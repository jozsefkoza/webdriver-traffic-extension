 ...
 * new entries can be captured in the traffic after two consecutive poll. Both the polling interval and the max amount of time of polling is configurable.
 * By default, traffic is polled for at most 10 seconds with 500 milliseconds intervals.
 *
 * @author JoeZee
 */
public final class PollingNetworkTrafficSource implements NetworkTrafficSource {
    private static final Logger LOG = LoggerFactory.getLogger(PollingNetworkTrafficSource.class);
    ...
    private final NetworkTrafficSource trafficSource;
    ...
    public PollingNetworkTrafficSource(NetworkTrafficSource trafficSource) {
    ...
    private Snapshot pollTraffic() {
        ...
        return Flowable.intervalRange(0, getPollCount(), 0, pollingInterval, TimeUnit.MILLISECONDS)
                ...
                .takeWhile(snapshot -> snapshot.capturedElementCount() > latestSnapshot.getAndSet(snapshot).capturedElementCount())
                .blockingLast();
    }
    ...
    private static final class Snapshot {
        ...
        static Snapshot create(NetworkTrafficSource trafficSource) {
